...
const Button = forwardRef(({className, ...otherProps }, ref) =>
  <button
...
All is well and good—you have a `Button` component that matches the UI your designer’s specs. You turn to your next task, which involves adding a feature that includes the following UI:

[ADD IMAGE OF FIGMA SPECS THAT DEPICT A BUTTON USED AS A LINK HERE]

… But wait! Your designers have used that `Button` as a link. But it’s not a link — I mean, it certainly _looks_ like a button. And the component we’ve built to represent this UI is called a `Button`. And that component renders `<button>` element … But! Despite all of this, a clickable element that takes you to a new page is a link, not a button.
...
Component reusability is at the heart of React development, and polymorphism is topic that I think is pretty important when considering how to build a robust React component. While a few of you out there may be scratching your head and wondering what I’m talking about, if you’ve spent much time working with React, it’s likely you’ve encountered the patterns I’m about to describe.

If you’ve used CSS-in-JS libraries like `styled-components`, or popular component libraries such as Material UI, you may have seen the “as” prop. If that isn’t ringing any bells, I’m referring to a pattern like so:
...
const Button = forwardRef(({className, ...otherProps }, ref) => {
  ...
  return <Component className={cx(buttonStyles, className)} ref={ref} {...otherProps} />;
});
```

This implementation works, but what it lacks proper type annotation. Fortunately, there are a number of packages that specifically provide types for this usecase, such as [react-polymorphed](https://github.com/nasheomirro/react-polymorphed), and if only for ease of use, I’d generally recommend just using those to add proper type annotations for your component, but if you’re interested in defining the types yourself, then read on.
...
Here, the `Button` component has been provided both an `asChild` prop, and an `<a>` tag as the root child. In this implementation, the `Button` will _clone_ the root node, and add the additional props controlled by the `Button` component onto the root child. A simple, component-specific implementation may look something like:

```jsx
const Button = forwardRef(({className, children, ...otherProps }, ref) => {
...
So which is better? The answer, as with most things, is that it depends. Polymorphic components are a powerful tool, but the patterns used to implement them are best understood as tools in your belt, and understanding which is most appropriate is a question only you can answer for your specific situation.

There is a real value in the `asChild` pattern for Library developers, who may have a wide, distributed audience and a need to support many usecases involving complex compositions of polymorphic components.

A lot of my day-to-day work is on _specific_ apps, though, and they often have clear expectations and needs for UI and feature-set. In these scenarios, the shortcomings of the `as` prop are rarely encountered or easily worked-around. I also have a simple preference for the _terseness_ of the `as` prop, compared to the additional JSX and nesting of `asChild`.

But a preference is itself not a commandment, and I’m more than happy to adapt to the needs of a given project. What I hope you take away from this article is not so much an advocacy for a specific implementation, but an understanding of the value of polymorphic components, and how they allow you to streamline the development process by writing more adaptable and maintainable components.
